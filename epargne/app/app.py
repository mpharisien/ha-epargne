...
def pea_import_csv():
    ...
    for i, row in enumerate(reader, 2):
        try:
            conn.execute("INSERT INTO pea_mouvements (date,type,nom_action,ticker,quantite,prix_unitaire,montant,notes) VALUES (?,?,?,?,?,?,?,?)",
                (row["date"].strip(), row["type"].strip().lower(),
                 row.get("nom_action","").strip() or None,
                 row.get("ticker","").strip() or None,
                 float(row["quantite"]) if row.get("quantite","").strip() else None,
                 float(row["prix_unitaire"]) if row.get("prix_unitaire","").strip() else None,
                 float(row["montant"]), row.get("notes","").strip()))
            nb += 1
